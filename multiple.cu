...
__device__ void
warp(volatile dtype* input, int index,unsigned int n)
{	/*
	if(n > 64){
	input[index] += input[index + 32];}
	input[index] += input[index + 16];
...
kernel5(dtype *g_idata, dtype *g_odata, unsigned int n)
{
	...
	unsigned int block = blockDim.x * blockDim.y * blockDim.z;
	unsigned int bid = gridDim.x * blockIdx.y + blockIdx.x;
	unsigned int i =2* bid * block + threadIdx.x;
	unsigned int grid = block * 2 * gridDim.x * gridDim.y * gridDim.z;

	scratch[threadIdx.x] = 0;
	while(i < n)
	{
		scratch[threadIdx.x] = scratch[threadIdx.x] + g_idata[i] + g_idata[i + block];
		i+=grid;
	}
	__syncthreads();
	unsigned int s;
	...
	if(threadIdx.x < 32)
		warp(scratch,threadIdx.x,n);
	/*
	while(threadIdx.x < 32 && s > 0)
	{
		scratch[threadIdx.x] +=scratch[threadIdx.x+s];
		s = s >> 1;
	}*/
...
main(int argc, char** argv)
{
  ...
  if(argc > 1) {
	...
	/* execute kernel */
	kernel5 <<<gb, tb>>> (d_idata, d_odata, N);
	fprintf(stdout,"Here s is %d,blocks is %d,threads is %d\n",N,blocks,threads );
	int s = blocks;
//	fprintf(stdout,"Here s is %d,blocks is %d,threads is %d\n",s,blocks,threads );
	while(s > 1) {
		threads = 0;
