...
void getNumBlocksAndThreads(int whichKernel, int n, int maxBlocks, int maxThreads, int &blocks, int &threads)
{
  if (whichKernel < 3)
    {
     ...
     // fprintf(stdout,"in if n is %d, threads is %d\n",n,threads);
      blocks = (n + threads - 1) / threads;
    //  fprintf(stdout,"so blocks is %d\n",blocks);
    }
  else
    {
  ...
  // fprintf(stdout,"here s is %d,threads is %d,blocks is %d\n",n,threads,blocks);
}
...
kernel0 (dtype *input, dtype *output, unsigned int n)
{
 ...
 // printf("initial param\n");
  //printf("griddim.x is %d, blockIdx.y is %d,blockIdx.x is %d,blockDim.x is %d,threadidx.x is %d\n",gridDim.x,blockIdx.y,blockIdx.x,blockDim.x,threadIdx.x);
...
main(int argc, char** argv)
{
 ...
 // fprintf(stdout,"the value of blockIdx is %d",blockIdx.x);
  ...
  kernel0 <<<gb, tb>>> (d_idata, d_odata, N);
  int s = blocks;
 //fprintf(stdout,"blocks num is %d\n",s);
// printf("threads is %d\n",threads);
   while(s > 1) {
    threads = 0;
    blocks = 0;
    getNumBlocksAndThreads (whichKernel, s, MAX_BLOCKS, MAX_THREADS, 
			    blocks, threads);
   // fprintf(stdout,"here threads is %d,block is %d",threads,blocks);
    dim3 gb(16, (blocks + 16 - 1) / 16, 1);
    dim3 tb(threads, 1, 1);
