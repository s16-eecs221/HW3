...
main(int argc, char** argv)
{
  ...
  if(argc > 1) {
	...
	/* naive kernel */
	whichKernel = 4;
	getNumBlocksAndThreads (whichKernel, bound, MAX_BLOCKS, MAX_THREADS, 
													blocks, threads);
	...
	int s;
	double temp=0;	
	for(int x = 0;x < N;x += bound)
	{
	getNumBlocksAndThreads (whichKernel, bound, MAX_BLOCKS, MAX_THREADS,blocks, threads);	
	/* execute kernel */
	dim3 gb(blocks, 1, 1);
        dim3 tb(threads, 1, 1);
	kernel4 <<<gb, tb>>> (&(d_idata[x]), d_odata, bound);
